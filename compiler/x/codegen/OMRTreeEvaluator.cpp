...
TR::Register* OMR::X86::TreeEvaluator::performExchangeNative(TR::Node* node, int8_t size, TR_X86OpCodes op, TR::CodeGenerator* cg)
   {
...
TR::Register *OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (SymRef && SymRef->getSymbol()->castToMethodSymbol()->isInlinedByCG())
      {
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicAddIntSymbol))
         {
         return performExchangeNative(node, 4, LADD4MemReg, cg);
         }
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicFetchAndAddIntSymbol))
         {
         return performExchangeNative(node, 4, LXADD4MemReg, cg);
         }
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicSwapIntSymbol))
         {
         return performExchangeNative(node, 4, XCHG4MemReg, cg);
         }
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicAddLongSymbol))
         {
         return performExchangeNative(node, 8, LADD8MemReg, cg);
         }
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicFetchAndAddLongSymbol))
         {
         return performExchangeNative(node, 8, LXADD8MemReg, cg);
         }
      if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicSwapLongSymbol))
         {
         return performExchangeNative(node, 8, XCHG8MemReg, cg);
         }
      }
